...
class Conv(Layer):
  ...
  def __init__(self, rank, my_filter,
               filters,
               kernel_size,
               strides=1,
               padding='valid',
  ...
  def call(self, inputs):
    ...
    conv_op = nn_ops.Convolution(
        ...
        data_format=conv_utils.convert_data_format(self.data_format,
                                                   self.rank + 2))
    print(type(self.kernel), self.kernel.shape)
...
class MyConv2D(Conv2D, base.Layer):
  """2D convolution layer (e.g. spatial convolution over images).
  This layer creates a convolution kernel that is convolved
  (actually cross-correlated) with the layer input to produce a tensor of
  outputs. If `use_bias` is True (and a `bias_initializer` is provided),
  ...
  def __init__(self,my_filter, filters,
               kernel_size,
               strides=(1, 1),
               padding='valid',
...
def my_conv2d(inputs,
  ...
  Arguments:
    ...
    strides: An integer or tuple/list of 2 integers,
      ...
      any `dilation_rate` value != 1.
    padding: One of `"valid"` or `"same"` (case-insensitive).
    data_format: A string, one of `channels_last` (default) or `channels_last`.
      The ordering of the dimensions in the inputs.
      `channels_last` corresponds to inputs with shape
      `(batch, height, width, channels)` while `channels_last` corresponds to
      inputs with shape `(batch, channels, height, width)`.
    dilation_rate: An integer or tuple/list of 2 integers, specifying
      the dilation rate to use for dilated convolution.
      Can be a single integer to specify the same value for
